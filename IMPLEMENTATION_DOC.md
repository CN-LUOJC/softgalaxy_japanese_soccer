...
The current implementation supports **all EVM chains** without a hardcoded limit. Known chains (listed below) show rich metadata; unknown chains gracefully display their chain ID. Non-EVM chains are not supported — see §2 Libraries.
...
**Known chains (15 — all EVM chains work, known ones show rich metadata):**
...
There is no "unsupported EVM chain" concept — any EVM chain the wallet connects to is accepted. Unknown chains display their chain ID without warnings.
...
The dropdown already shows the last 5 transactions for the connected address on known chains by querying the block explorer API. Future work could add pagination, token transfers, and decoded transaction details.
...
`useJPSoccerSignMessage` hook and `web3.ts` utilities (`shortenAddress`, `CHAIN_METADATA`) have 17 unit tests running via Vitest. Playwright (already a project dependency) could be used to add E2E wallet connection tests using a test wallet extension.
